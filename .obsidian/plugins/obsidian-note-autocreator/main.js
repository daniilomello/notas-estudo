...
var SettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  addTemplateTriggerSetting(containerEl) {
    new import_obsidian3.Setting(containerEl).setName("Trigger for template execution").setDesc("The text string that will trigger template execution").addText((component) => component.setValue(this.plugin.settings.templateTriggerSymbol).onChange((value) => __async(this, null, function* () {
...
var ObsidianInterop = class {
  ...
  folderExists(folderPath) {
    ...
    return foundItem && foundItem instanceof import_obsidian4.TFolder;
  }
  noteExists(notePath) {
    const foundItem = app.metadataCache.getFirstLinkpathDest(notePath, app.workspace.getActiveFile().path);
    return foundItem && foundItem instanceof import_obsidian4.TFile;
  }
  getAllFileDescendantsOf(folderPath) {
    ...
    const files = [];
    import_obsidian4.Vault.recurseChildren(abstractFile, (file) => {
      if (file instanceof import_obsidian4.TFile) {
        files.push(file);
      }
    });
  ...
  getPathsToAllLoadedFolders() {
    return app.vault.getAllLoadedFiles().filter((f) => f instanceof import_obsidian4.TFolder).map((f) => new ObsidianFolderPath(f.path));
  }
...
var TemplateSuggestion = class extends FileSuggestion {
  constructor(templatePath, noteSuggestion, rootTemplateFolder) {
    super(templatePath);
    this.triggerSymbol = "$";
...
var TemplateSuggestionCollector = class {
  constructor(fileSystem, configStore, settings) {
    this.fileSystem = fileSystem;
    this.configStore = configStore;
  ...
  getAllPossibleSuggestions(templateFolderPath, noteSuggestion) {
    return templateFolderPath ? this.fileSystem.getAllFileDescendantsOf(templateFolderPath).map((f) => new TemplateSuggestion(f.path, noteSuggestion, templateFolderPath)) : [];
  }
};
...
var LinkSuggestorInterop = class extends import_obsidian6.EditorSuggest {
  constructor(app2, settings) {
    super(app2);
    this.wrapped = new LinkSuggestor(new ObsidianInterop(app2), settings);
...
// src/main.ts
var NoteAutoCreator = class extends import_obsidian7.Plugin {
  onload() {
    return __async(this, null, function* () {
